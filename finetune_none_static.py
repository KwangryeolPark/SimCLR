...
if __name__ == "__main__":
    ...
    wandb.init(
        project='SCL',
        tags=['original_code', 'finetune', 'none_static'],
        name=str("static" if args.save_static else "none_static") + " finetune",
        config=args
    )
    ...
    for epoch in range(args.logistic_epochs):
        loss_epoch, accuracy_epoch = train(
            args, train_loader, simclr_model, None, criterion, optimizer
        )
        print(
        ...
        wandb.log({
            ...
            "train loss": loss_epoch / len(train_loader)
        })

    # final testing
        loss_epoch, accuracy_epoch = test(
            args, test_loader, simclr_model, None, criterion, optimizer
        )
        print(
            f"[FINAL]\t Loss: {loss_epoch / len(test_loader)}\t Accuracy: {accuracy_epoch / len(test_loader)}"
        )
        wandb.log({
            'test acc': accuracy_epoch / len(test_loader),
            'test loss': loss_epoch / len(test_loader)
        })
